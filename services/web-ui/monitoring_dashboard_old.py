...
class MonitoringDashboard:
    ...
    def sidebar_controls(self):
        ...
        symbol = st.sidebar.selectbox("Symbol", ["BTC/USD", "ETH/USD", "EUR/USD"], key="monitor_symbol")
        timeframe = st.sidebar.selectbox("Timeframe", ["1m", "5m", "1h", "1d"], key="monitor_timeframe")
        
        # Alert thresholds
        st.sidebar.subheader("🚨 Alert Thresholds")
        win_rate_threshold = st.sidebar.slider("Win Rate Alert (%)", 0, 100, 60, key="win_rate_alert")
        drawdown_threshold = st.sidebar.slider("Max Drawdown Alert (%)", 0, 50, 20, key="drawdown_alert")
    ...
    def main_metrics_section(self):
        ...
        if metrics:
            ...
            with col1:
                ...
                color = "🟢" if win_rate >= st.session_state.get('win_rate_threshold', 60) else "🔴"
                st.metric("Win Rate", f"{win_rate:.1f}%", delta=None, delta_color="normal")
                st.caption(f"{color} Target: {st.session_state.get('win_rate_threshold', 60)}%")
            ...
            with col4:
                ...
                color = "🟢" if max_drawdown < st.session_state.get('drawdown_threshold', 20) else "🔴"
                st.metric("Max Drawdown", f"{max_drawdown:.1f}%", delta=None, delta_color="normal")
                st.caption(f"{color} Limit: {st.session_state.get('drawdown_threshold', 20)}%")
    ...
    def check_alerts(self):
        ...
        if metrics:
            ...
            # Win rate alert
            win_rate = metrics.get('win_rate', 0)
            threshold = st.session_state.get('win_rate_threshold', 60)
            ...
            # Drawdown alert
            max_drawdown = metrics.get('max_drawdown', 0)
            dd_threshold = st.session_state.get('drawdown_threshold', 20)
