...
try:
    from .commodities import CommoditiesDataSource
    COMMODITIES_AVAILABLE = True
except ImportError:
    COMMODITIES_AVAILABLE = False

try:
    from .futures import FuturesDataSource
...
class DataSourceFactory:
    ...
    def create_data_source(name: Optional[str] = None,
        ...
        if name == 'yahoo_finance' or name == 'yahoo':
            ...
            if not FOREX_AVAILABLE:
                raise ValueError("Forex data source is not available.")
            return ForexDataSource(config)

        elif name == 'commodities' or name == 'commodity':
            if not COMMODITIES_AVAILABLE:
                raise ValueError("Commodities data source is not available.")
            return CommoditiesDataSource(config)

        elif name == 'futures' or name == 'future':
            if not FUTURES_AVAILABLE:
                raise ValueError("Futures data source is not available.")
            return FuturesDataSource(config)
    ...
    def get_available_sources() -> Dict[str, bool]:
        ...
        return {
            ...
            'commodities': COMMODITIES_AVAILABLE,
