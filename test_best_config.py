...
def main():
    print('🧪 Testing Best Discovery Configuration in Backtest Mode')
    ...
    try:
        ...
        print('✅ Loaded best configuration from discovery results')
        ...
        print('⚠️ Using manually defined best configuration as fallback')

    # Calculate Buy & Hold performance
    print('Calculating Buy & Hold performance...')
    bh = calculate_buy_hold_performance('BTC/USDT', 'kraken', '1h')
    print(f'📊 Buy & Hold Performance (Benchmark): {bh:.2f}%')
    print()
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Signals prepared: {data['Buy_Signal'].sum()} buy, {data['Sell_Signal'].sum()} sell")
            
            trades = backtester._simulate_trades(data)
            backtester.trades = trades
            print(f"✅ Trades simulated: {len(trades)} trades")
        ...
        if 'backtest_results' in results and 'error' not in results['backtest_results']:
            ...
            if vs_benchmark > 0:
                print('🎉 SUCCESS: Configuration outperforms Buy & Hold!')
            else:
                print('📉 Configuration underperforms Buy & Hold')
