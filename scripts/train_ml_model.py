...
def train_single_ml_model(symbol: str = SYMBOL, timeframe: str = TIMEFRAME,
                          model_name: str = 'random_forest', lookback_days: int = LOOKBACK_DAYS,
                          force_retrain: bool = False):
    """
    Train a single ML model for signal prediction.
    ...
    Args:
        ...
        force_retrain: Force retraining even if model exists
    """
    print(f"🤖 Training single ML model '{model_name}' for {symbol} on {timeframe} timeframe")
    print(f"   Historical data: {lookback_days} days")
    print(f"   Force retrain: {force_retrain}")
    print("-" * 60)
    ...
    try:
        ...
        print(f"🎯 Training single ML model: {model_name}...")
        training_result = ml_predictor.train_single_model(
            df_clean,
        ...
        if training_result['success']:
            print("✅ Single ML model training completed!")
            print(f"   Model: {training_result['best_model']}")
            print(f"   Training samples: {training_result['samples']}")
            print(f"   Features used: {training_result['features']}")

            # Show performance summary
            performance = training_result['performance']
            if model_name in performance and 'f1_score' in performance[model_name]:
            ...
            print(f"❌ Single ML training failed: {training_result.get('error', 'Unknown error')}")
            return False
...
def main():
    ...
    parser.add_argument('--model', choices=['random_forest', 'gradient_boosting', 'xgboost', 'lightgbm', 'svm', 'logistic_regression'],
                       help='Train only a specific ML model instead of all models')
    ...
    if args.evaluate:
        ...
        success = train_single_ml_model(args.symbol, args.timeframe, args.model, args.days, args.force)
        if success:
            print(f"\n🎉 Single ML model '{args.model}' training completed successfully!")
            print("The model will now enhance trading signals with ML predictions.")
        else:
            print(f"\n❌ Single ML model '{args.model}' training failed.")
            sys.exit(1)
    else:
