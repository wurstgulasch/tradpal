...
class TestDataService:
    ...
    @pytest.fixture
    def data_service(self, event_system):
    ...
    def test_metadata_serialization(self, data_service):
...
class TestDataServicePerformance:
    ...
    async def test_data_fetch_performance(self, data_service):
        ...
        with patch.object(data_service, '_fetch_from_ccxt', new_callable=AsyncMock) as mock_fetch:
            ...
            # Simple timing instead of benchmark
            import time
            start_time = time.time()
            result = await data_service.fetch_data(request)
            end_time = time.time()
